...
| 吸收轨 | TradingAgents/CN 工程纪律 | 进行中 | 唯一方案 [adoption_priority_plan.md](adoption_priority_plan.md)；阶段 A+B 已绿，下一阶段 C（decision_log） |
...
1. **吸收轨阶段 C**：`decision_log` pending→结算；门禁见 [adoption_priority_plan.md](adoption_priority_plan.md) GATE-C1。阶段 A/B 已绿。  
