...
| 吸收轨 | TradingAgents/CN 工程纪律 | **主干完成** | A–D 全绿；E1 schema + E2 只读助手已落地；E3/财务全量/P4 仍后置 |
...
1. **吸收轨收尾**：财务全量拉数（X3）、E3 新闻/多周期仅在有产品需求时开；可选接真实 LLM 到 `draft_assistant.llm_fn`。  
...
6. **Record/Event**：新事件类型先扩 `RECORD_SCHEMAS` + normalizer + fixture；财务全量仍属 X3（schema 已预留 `fundamental_snapshot`）。  
7. **Q1**：付费数据源仍暂缓。
