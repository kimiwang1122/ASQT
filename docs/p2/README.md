...
Q3 组合：**规则择时（ETF 均线轮动）+ A 股动量 TopK + ETF 动量 TopK**。引擎是 `LocalResearchEngine`（实现 `ResearchEngine`），不在业务层 import `qlib`。
...
1. 三个策略都产出报告：`etf_ma_rotate`（rule）、`stock_momentum_topk`（topk）、`etf_momentum_topk`（topk，40 日）
